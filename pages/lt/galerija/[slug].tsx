...
export default function ArtworkDetailLt({ artwork, related }: Props) {
  const { addProduct, cartProducts } = useCart();
  const [added, setAdded] = useState(false);

  // Defensive guard: during navigation transitions or hot reload, artwork may briefly be undefined.
  ...
  function handleAddToCart() {
    ...
    addProduct({
      ...
      stripePriceId: artwork.id, // artwork uses price_data in checkout, not a Stripe price ID
  ...
  return (
    <>
      <Head>
        ...
        <link rel='canonical' href={`https://www.paulapango.com/lt/galerija/${artwork.slugLt}`} />
        <link rel='alternate' hrefLang='en' href={`https://www.paulapango.com/gallery/${artwork.slug}`} />
        <link rel='alternate' hrefLang='lt-LT' href={`https://www.paulapango.com/lt/galerija/${artwork.slugLt}`} />
      </Head>
      ...
      <div className='max-w-6xl mx-auto px-6 py-16'>
        ...
        <nav className={`text-xs text-text-muted mb-10 flex items-center gap-2 ${jost.className}`}>
          ...
          <span>/</span>
          <Link href='/lt/galerija' className='hover:text-primary transition-colors'>Galerija</Link>
          <span>/</span>
        ...
        {related.length > 0 && (
          <section className='mt-24'>
            ...
            <div className='flex flex-wrap justify-center gap-5'>
              {related.map((rel) => (
                <Link key={rel.id} href={`/lt/galerija/${rel.slugLt}`} className='group block w-[calc(50%-10px)] md:w-[calc(25%-15px)]'>
